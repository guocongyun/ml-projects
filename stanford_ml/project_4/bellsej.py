...
def load_data():
    mix = np.loadtxt('../data/project_4/mix.dat')
    return mix
...
def unmixer(X):
    ...
    for learning_rate in anneal:
        ...
        for row in X:
            row = row.reshape(5,1)
            # IMPORTANT matrices in numpy are row matrices, default 1,5 as aooise to column matrices in papers 5,1
    ...
    with open('../data/project_4/ICA_weights','w+b') as weight_file: pickle.dump(W, weight_file)
...
def main():
    ...
    with open('../data/project_4/ICA_weights','rb') as weight_file: W = pickle.load(weight_file)
