...
###### BEGIN YOUR CODE ######
# TODO:
trans_record = np.zeros((NUM_STATES, NUM_ACTIONS, NUM_STATES))
reward_record = np.zeros(NUM_STATES)
rewards_counts = np.zeros(NUM_STATES)

values = np.random.uniform(0, 0.1, NUM_STATES)
# Very important, by default (x,) shape == (1,x) != (x,1) hence np uses row matrixs
rewards = np.zeros(NUM_STATES)
trans_prob = np.full((NUM_STATES, NUM_ACTIONS, NUM_STATES), 1/NUM_STATES)
# IMPORTANT trans_prob is 3d Array
...
while consecutive_no_learning_trials < NO_LEARNING_THRESHOLD:
    ...
    action = np.argmax(np.sum(trans_prob[state] * values, axis=1))
    # print(np.sum(trans_prob[state] * values, axis=1))
    ###### END YOUR CODE ######
    ...
    ###### BEGIN YOUR CODE ######
    # TODO:
    trans_record[state, action, new_state] +=1
    reward_record[new_state] = R
    rewards_counts[new_state] += 1
    ...
    if new_state == NUM_STATES - 1:
        ...
        ###### BEGIN YOUR CODE ######
        # TODO:
        mask = trans_record.sum(axis=2) > 0 
        trans_prob[mask] = trans_record[mask] /  trans_record.sum(axis=2)[mask].reshape(-1,1)
        
        mask = rewards_counts > 0
        rewards[mask] = reward_record[mask] / rewards_counts[mask]
        ...
        while True:
            count += 1
            temp = values.copy()
            # IMPORTANt temp = values[:] this method have limitation with collection of mutable objects, or with np array as [1:3] in np array create a reference
            # for num in range(NUM_STATES):
            #     values[num] = rewards[num] + np.argmax(GAMMA * (trans_prob[num] @ temp.T))
            values = rewards + GAMMA * np.max(trans_prob @ temp.T, axis=1)
            # mask = np.sum(trans_prob * temp, axis=2) != trans_prob @ temp.T
            # print(np.sum(trans_prob * temp, axis=2)[mask][:5])
            # print((trans_prob @ temp.T)[mask][:5])
            # IMPORTANT + max not argmax
            delta = max(np.abs(values - temp))
            if delta < TOLERANCE:
                break
