...
# IMPORTANT create a ml template in this structure
...
def forward_prop(data, labels, params):
    ...
    # IMPORTANT assert(False) = False, assert((False,)) == True isn't tuple evaluate to true if have content  
...
def backward_prop(data, labels, params, lambd=0):
    ...
    # VERY IMPORTANT here we have already sum the 1000 data points via 300x1000 @ 1000x700 dot product
...
#%%
    ...
    for run in range(epoch_num): 
        ...
        with open("./stanford_ml/prob_4/parameter_reg_2","w+b") as parameter: pickle.dump(params, parameter)
        # IMPORTANT add + in open mode to allow creating file, add b in open mode to open in binary mode for pickle
        with open("./stanford_ml/prob_4/stats_reg_2","w+b") as stat: pickle.dump(stats, stat)

            # for key, value in params.items(): params_[key] = value.tolist(); parameter.write(str(params_))
#%%
    with open("./stanford_ml/prob_4/stats","rb") as stat: stats = pickle.load(stat)
    with open("./stanford_ml/prob_4/stats_reg","rb") as stat: stats_reg = pickle.load(stat)
    with open("./stanford_ml/prob_4/stats_reg_2","rb") as stat: stats_reg_2 = pickle.load(stat)
#%%
...
#%%
    np.random.seed(100)
    trainData, trainLabels = readData('./stanford_ml/prob_4/images_train.csv', './stanford_ml/prob_4/labels_train.csv')
    ...
    testData, testLabels = readData('./stanford_ml/prob_4/images_test.csv', './stanford_ml/prob_4/labels_test.csv')
...
#%%
    ...
    if readyForTesting:
        with open("./stanford_ml/prob_4/parameter","rb") as parameter: params = pickle.load(parameter)
        with open("./stanford_ml/prob_4/parameter_reg","rb") as parameter: params_reg = pickle.load(parameter)
        with open("./stanford_ml/prob_4/parameter_reg_2","rb") as parameter: params_reg_2 = pickle.load(parameter)
