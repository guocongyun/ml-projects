...
xx, yy = np.linspace(-2, 20,100), np.linspace(-5, 5,100) # IMPORTANT, xx, yy === x1, x2
...
for iy in range(XX.shape[0]): # IMPORTANT, np array.shape[0] ==  len(array), nparray.shape[1] == len(array[0])
...
P = cvxopt.matrix(np.array([[1.5, 1],
              [1.0, 0.5]]))

q = cvxopt.matrix(np.array([[1.0], 
              [6.0]]))

G = cvxopt.matrix(np.array([
    [-2.0, 3.0],
    [-1.0, 0.0],
    [0.0, -1.0],
]))
    
h = cvxopt.matrix(np.array([[4.0], 
              [0.0], 
              [0.0]]))

print(cvxopt.solvers.qp(P, q, G, h)["x"])