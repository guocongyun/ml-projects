...
class CharDecoder(nn.Module):
    ...
    def forward(self, input, dec_hidden=None):
        ...
        char_embeds = self.decoderCharEmb(input.long())  # Tensor: (l, b, e_char)
    ...
    def decode_greedy(self, initialStates, device, max_length=21):
        ...
        for _ in range(max_length):
            ...
            prob = torch.softmax(scores.squeeze(0), dim=1) # (1, b, vocab_len)
            current_char_ids = torch.argmax(prob, dim=1).unsqueeze(0) # (1, b)
            for idx, char in enumerate(current_char_ids[0]):
                output_words[idx] += self.target_vocab.id2char[int(char)]

        #IMPORTANT decoded words !===== output_words
        decodedWords = []
