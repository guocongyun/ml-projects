...
def distance_to_opponent(obs, player_x, player_y, steps=0):
    ...
    for i in range(1, len(obs["right_team"])):
        ...
        distances_score -= 10**((10-step_to_opponent+steps)*damping_factor/10)
...
def distance_to_goal(obs, player_x, player_y, steps=0):
    ...
    distances_score = 10**(1-distance_to_goal+unit_steps)
    return distances_score
...
def bad_angle_short_pass(obs, player_x, player_y):
    ...
    def get_action(obs, player_x, player_y):
        ...
        if is_blocked(obs, player_x, player_y, 1, 0, SHOT_SPEED, opponent_speed=1) and is_blocked(obs, player_x, player_y, 1, 0.044, SHOT_SPEED, opponent_speed=1) and is_blocked(obs, player_x, player_y, 1, -0.044, SHOT_SPEED, opponent_speed=1): return Action.HighPass
        else: return Action.Shot
...
def close_to_opponent_short_pass(obs, player_x, player_y):
    ...
    def environment_fits(obs, player_x, player_y):
        ...
        for j in range(0, len(obs["left_team"])):
            ...
            if 1/6 < distance_to_teammate < 1/2:
...
def go_through_opponents(obs, player_x, player_y):
    ...
    def environment_fits(obs, player_x, player_y):
        ...
        biggest_distance = get_player_score(obs, player_x + 0.01, player_y)
        obs["memory_patterns"]["go_around_opponent"] = Action.Right
        # if top right direction is safest
        top_right = get_player_score(obs, player_x + 0.01, player_y - 0.01)
        ...
        bottom_right = get_player_score(obs, player_x + 0.01, player_y + 0.01)
