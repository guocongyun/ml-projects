...
def is_within_border(obs, player_x, player_y, xborder = None, yborder = None):
    if xborder != None: distance_to_xborder = abs(xborder - player_x)
    else: distance_to_xborder = 0
    if yborder != None: distance_to_yborder = abs(yborder - player_y)
    else: distance_to_yborder = 0
...
def get_player_score(obs, player_x, player_y, steps=0):
    player_score = 0
    player_score += distance_to_opponent(obs, player_x, player_y, steps)
    player_score += distance_to_goal(obs, player_x, player_y, steps/2)
    return player_score

def evaluation(obs, player_x, player_y, teammate_x, teammate_y, passing_steps=SHORT_STEPS):
    if passing_steps != HIGH_STEPS: 
        if is_blocked(obs, player_x, player_y, teammate_x, teammate_y): return False
    if is_illigal_move(obs, player_x, player_y): return False
    
    player_steps_to_teammate = check_steps(player_x, player_y, teammate_x, teammate_y)

    player_score = get_player_score(obs, player_x, player_y, steps=player_steps_to_teammate)
...
def bad_angle_short_pass(obs, player_x, player_y):
    ...
    def get_action(obs, player_x, player_y):
        ...
        if is_blocked(obs, player_x, player_y, 1, 0, SHOT_SPEED, opponent_speed=1) and is_blocked(obs, player_x, player_y, 1, 0.42, SHOT_SPEED, opponent_speed=1) and is_blocked(obs, player_x, player_y, 1, -0.42, SHOT_SPEED, opponent_speed=1): return Action.HighPass
        else: return Action.ShortPass
...
def go_through_opponents(obs, player_x, player_y):
    ...
    def environment_fits(obs, player_x, player_y):
        ...
        right_score = get_player_score(obs, player_x + RUN_SPEED, player_y, steps=0)
        top_right_score = get_player_score(obs, player_x + math.sqrt(RUN_SPEED), player_y - math.sqrt(RUN_SPEED), steps=0)
        bottom_right_score = get_player_score(obs, player_x + math.sqrt(RUN_SPEED), player_y + math.sqrt(RUN_SPEED), steps=0)
        top_score = get_player_score(obs, player_x, player_y - RUN_SPEED,steps=1/2)
        bottom_score = get_player_score(obs, player_x, player_y + RUN_SPEED,steps=1/2)
        top_left_score = get_player_score(obs, player_x - math.sqrt(RUN_SPEED), player_y - math.sqrt(RUN_SPEED), steps=1)
        bottom_left_score = get_player_score(obs, player_x - math.sqrt(RUN_SPEED), player_y + math.sqrt(RUN_SPEED), steps=1)
        left_score = get_player_score(obs, player_x - RUN_SPEED, player_y,steps=3/2)

        possible_movement = [-999, -999, -999, -999, -999, -999, -999, -999]
        
        if is_within_border(obs, player_x, player_y, xborder=1, yborder=None): possible_movement[0] = right_score
        if is_within_border(obs, player_x, player_y, xborder=1, yborder=-0.42): possible_movement[1] = top_right_score
        if is_within_border(obs, player_x, player_y, xborder=1, yborder=0.42): possible_movement[2] = bottom_right_score
        if is_within_border(obs, player_x, player_y, xborder=None, yborder=-0.42): possible_movement[3] = top_score
        if is_within_border(obs, player_x, player_y, xborder=None, yborder=0.42): possible_movement[4] = bottom_score
        if is_within_border(obs, player_x, player_y, xborder=-0.75, yborder=-0.42): possible_movement[5] = top_left_score
        if is_within_border(obs, player_x, player_y, xborder=-0.75, yborder=0.42): possible_movement[6] = bottom_left_score
        if is_within_border(obs, player_x, player_y, xborder=-0.5, yborder=None): possible_movement[0] = left_score
