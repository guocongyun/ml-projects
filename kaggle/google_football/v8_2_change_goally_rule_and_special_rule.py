...
def close_to_goalkeeper_shot(obs, player_x, player_y):
    ...
    def environment_fits(obs, player_x, player_y):
        ...
        for step in range(9):
            ...
            if get_distance(player_x_, player_y_, goalkeeper_x, goalkeeper_y) < RUN_SPEED:
                return True
        if get_distance(player_x, player_y, 1, 0) <= 0.5:
            if not is_blocked(obs, player_x, player_y, 1, 0, SHOT_SPEED, opponent_speed=1):
                return True
            elif not is_blocked(obs, player_x, player_y, 1, 0.044, SHOT_SPEED, opponent_speed=1):
                return True
            elif not is_blocked(obs, player_x, player_y, 1, -0.044, SHOT_SPEED, opponent_speed=1):
                return True
        return False
