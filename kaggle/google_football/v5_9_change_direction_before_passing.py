...
def normal_get_distance_to_opponent(obs, player_x, player_y, steps=0):
    ...
    for i in range(1, len(obs["right_team"])):
        ...
        distance_to_opponent = normal_get_distance(player_x, player_y, obs["right_team"][i][0], obs["right_team"][i][1])
        if distance_to_opponent < RUN_SPEED*1: distances_score -= 9**(20+steps)*damping_factor/20; distances_amount += 1
        elif distance_to_opponent < RUN_SPEED*2: distances_score -=9**(19+steps)*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*3: distances_score -=9**(18+steps)*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*4: distances_score -=9**(17+steps)*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*5: distances_score -=9**(16+steps)*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*6: distances_score -=9**(15+steps)*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*7: distances_score -=9**(14+steps)*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*8: distances_score -=9**(13+steps)*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*9: distances_score -=9**(12+steps)*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*10: distances_score -=9**(11+steps)*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*11: distances_score -=9**(10+steps)*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*12: distances_score -=9**(9+steps)*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*13: distances_score -=9**(8+steps)*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*14: distances_score -=9**(7+steps)*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*15: distances_score -=9**(6+steps)*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*16: distances_score -=9**(5+steps)*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*17: distances_score -=9**(4+steps)*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*18: distances_score -=9**(3+steps)*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*19: distances_score -=9**(2+steps)*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*20: distances_score -=9**(1+steps)*damping_factor/20
    if distances_amount == 0:
...
def check_block(obs, player_x, player_y, teammate_x, teammate_y, speed=PASS_SPEED):
    ...
    for step in range(steps):
        ...
        for opponent in range(0, len(obs["right_team"])):
            distance_to_opponent = normal_get_distance(ball_pos[0], ball_pos[1], obs["right_team"][opponent][0], obs["right_team"][opponent][1])
...
def close_to_goalkeeper_shot(obs, player_x, player_y):
    ...
    def environment_fits(obs, player_x, player_y):
        ...
        for step in range(15):
            ...
            if normal_get_distance(player_x, player_y, goalkeeper_x, goalkeeper_y) < RUN_SPEED*2:
                return True
        if check_block(obs, player_x, player_y, 1, 0, SHOT_SPEED) and normal_get_distance(player_x, player_y, 1, 0) <= 0.5:
            return True
    ...
    def get_action(obs, player_x, player_y):
        """ get action of this memory pattern """
        if player_y < 0.15 and player_y > obs["right_team"][0][1]:
...
def close_to_opponent_short_pass(obs, player_x, player_y):
    ...
    def environment_fits(obs, player_x, player_y):
        ...
        for i in range(1, len(obs["right_team"])):
            distance_to_opponent = normal_get_distance(player_x, player_y, obs["right_team"][i][0], obs["right_team"][i][1])
            if distance_to_opponent < RUN_SPEED * (SHORT_STEPS + 3) and distance_to_opponent > RUN_SPEED * 2:
                for j in range(1, len(obs["left_team"])):
                    player_to_opponents = normal_get_distance_to_opponent(obs, player_x, player_y)
                    player_steps_to_teammate = check_steps(player_x, player_y, obs["left_team"][j][0], obs["left_team"][j][1])
                    teammate_to_opponents = normal_get_distance_to_opponent(obs, obs["left_team"][j][0], obs["left_team"][j][1], steps=player_steps_to_teammate)

                    distance_to_teammate = normal_get_distance(player_x, player_y, obs["left_team"][j][0], obs["left_team"][j][1])
                    if distance_to_teammate < 1/3 and player_to_opponents[0]*2 < teammate_to_opponents[0] and check_legal_move(obs, obs["left_team"][j][0], obs["left_team"][j][1]) and check_block(obs, player_x, player_y, obs["left_team"][j][0], obs["left_team"][j][1]):
                        teammate_distance_to_goal = normal_get_distance(obs["left_team"][j][0], obs["left_team"][j][1], 1, 0)
                        player_distance_to_goal = normal_get_distance(player_x, player_y, 1, 0)
                        if teammate_distance_to_goal < player_distance_to_goal - 1/6:
                            obs["memory_patterns"]["allie_direction"] = []
                            if obs["left_team"][j][0] > player_x: obs["memory_patterns"]["allie_direction"].append(Action.Right)
                            elif obs["left_team"][j][0] < player_x: obs["memory_patterns"]["allie_direction"].append(Action.Left)
                            elif obs["left_team"][j][1] < player_y: obs["memory_patterns"]["allie_direction"].append(Action.Top)
                            elif obs["left_team"][j][1] > player_y: obs["memory_patterns"]["allie_direction"].append(Action.Bottom)
                            elif obs["left_team"][j][0] > player_x and obs["left_team"][j][1] > player_y: obs["memory_patterns"]["allie_direction"].append(Action.BottomRight)
                            elif obs["left_team"][j][0] < player_x and obs["left_team"][j][1] > player_y: obs["memory_patterns"]["allie_direction"].append(Action.BottomLeft)
                            elif obs["left_team"][j][0] > player_x and obs["left_team"][j][1] < player_y: obs["memory_patterns"]["allie_direction"].append(Action.TopRight)
                            elif obs["left_team"][j][0] < player_x and obs["left_team"][j][1] < player_y: obs["memory_patterns"]["allie_direction"].append(Action.TopLeft)
                            return True
                            # Add check player current direction
                break
        return False
    ...
    def get_action(obs, player_x, player_y):
        ...
        if Action.Sprint in obs["sticky_actions"]:
            return Action.ReleaseSprint
        for action in obs["memory_patterns"]["allie_direction"]: 
            if action not in obs["sticky_actions"]: return action
        return Action.ShortPass
...
def close_to_opponent_long_pass(obs, player_x, player_y):
    ...
    def environment_fits(obs, player_x, player_y):
        ...
        for i in range(1, len(obs["right_team"])):
            distance_to_opponent = normal_get_distance(player_x, player_y, obs["right_team"][i][0], obs["right_team"][i][1])
            if distance_to_opponent < RUN_SPEED * (LONG_STEPS + 3)  and distance_to_opponent > RUN_SPEED * 2:
                for j in range(1, len(obs["left_team"])):
                    player_to_opponents = normal_get_distance_to_opponent(obs, player_x, player_y)
                    player_steps_to_teammate = check_steps(player_x, player_y, obs["left_team"][j][0], obs["left_team"][j][1])
                    teammate_to_opponents = normal_get_distance_to_opponent(obs, obs["left_team"][j][0], obs["left_team"][j][1], steps=player_steps_to_teammate)
                    ...
                    distance_to_teammate = normal_get_distance(player_x, player_y, obs["left_team"][j][0], obs["left_team"][j][1])
                    if distance_to_teammate < 2/3 and distance_to_teammate > 1/3 and player_to_opponents[0]*2 < teammate_to_opponents[0] and check_legal_move(obs, obs["left_team"][j][0], obs["left_team"][j][1]) and check_block(obs, player_x, player_y, obs["left_team"][j][0], obs["left_team"][j][1]):
                        teammate_distance_to_goal = normal_get_distance(obs["left_team"][j][0], obs["left_team"][j][1], 1, 0)
                        player_distance_to_goal = normal_get_distance(player_x, player_y, 1, 0)
...
def close_to_opponent_high_pass(obs, player_x, player_y):
    ...
    def environment_fits(obs, player_x, player_y):
        ...
        for i in range(1, len(obs["right_team"])):
            distance_to_opponent = normal_get_distance(player_x, player_y, obs["right_team"][i][0], obs["right_team"][i][1])
            if distance_to_opponent < RUN_SPEED * (HIGH_STEPS + 3) and distance_to_opponent > RUN_SPEED * 2:
                for j in range(1, len(obs["left_team"])):
                    player_to_opponents = normal_get_distance_to_opponent(obs, player_x, player_y)
                    player_steps_to_teammate = check_steps(player_x, player_y, obs["left_team"][j][0], obs["left_team"][j][1])
                    teammate_to_opponents = normal_get_distance_to_opponent(obs, obs["left_team"][j][0], obs["left_team"][j][1], steps=player_steps_to_teammate)
                    ...
                    distance_to_teammate = normal_get_distance(player_x, player_y, obs["left_team"][j][0], obs["left_team"][j][1])
                    if distance_to_teammate < 1 and distance_to_teammate > 1/3 and player_to_opponents[0]*2 < teammate_to_opponents[0] and check_legal_move(obs, obs["left_team"][j][0], obs["left_team"][j][1]):
                        teammate_distance_to_goal = normal_get_distance(obs["left_team"][j][0], obs["left_team"][j][1], 1, 0)
                        player_distance_to_goal = normal_get_distance(player_x, player_y, 1, 0)
                        if teammate_distance_to_goal < player_distance_to_goal - 2/3: # add don't pass back field rule
                            obs["memory_patterns"]["allie_direction"] = []
                            if obs["left_team"][j][0] > player_x: obs["memory_patterns"]["allie_direction"].append(Action.Right)
                            elif obs["left_team"][j][0] < player_x: obs["memory_patterns"]["allie_direction"].append(Action.Left)
                            elif obs["left_team"][j][1] < player_y: obs["memory_patterns"]["allie_direction"].append(Action.Top)
