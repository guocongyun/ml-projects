...
def distance_to_opponent(obs, player_x, player_y, steps=0):
    ...
    for i in range(1, len(obs["right_team"])):
        ...
        distances_score -= 10**((10-step_to_opponent+steps/2)*damping_factor/10)

    return distances_score

def distance_to_goal(obs, player_x, player_y, steps=0):
    distance_to_goal = get_distance(player_x, player_y, 0.9, -0.42/6)
    distance_to_goal = get_distance(player_x, player_y, 0.9, 0.42/6)

    unit_steps = (steps*RUN_SPEED)
    goal_score = 10**(1-distance_to_goal+unit_steps)
    distances_score = goal_score
    return distances_score

def distance_to_corner(obs, player_x, player_y, steps=0):
    distance_to_corner_up = get_distance(player_x, player_y, 0.9, -0.32)
    distance_to_goal_mid = get_distance(player_x, player_y, 1, 0)
    distance_to_corner_down = get_distance(player_x, player_y, 0.9, 0.32)
    distance_to_self_corner_down = get_distance(player_x, player_y, -1, 0.42)
    distance_to_self_corner_up = get_distance(player_x, player_y, -1, -0.42)

    unit_steps = (steps*RUN_SPEED)

    future_y = player_y + obs["left_team_direction"][obs["active"]][1]
    future_x = player_x + obs["left_team_direction"][obs["active"]][0]
    
    if future_y < 0:
        if future_x < -0.4:
            distances_score = 10**(1-distance_to_corner_up+unit_steps)*5/6 + 10**(1-distance_to_goal_mid+unit_steps)/6
        if future_x > -0.4:
            distances_score = 10**(1-distance_to_corner_up+unit_steps)*5/6 + 10**(1-distance_to_goal_mid+unit_steps)/6 + 10**(1-distance_to_self_corner_up+unit_steps)/6
    if future_y >= 0: 
        if future_x > -0.4:
            distances_score = 10**(1-distance_to_corner_down+unit_steps)*5/6 + 10**(1-distance_to_goal_mid+unit_steps)/6 
        if future_x < -0.4:
            distances_score = 10**(1-distance_to_corner_down+unit_steps)*5/6 + + 10**(1-distance_to_self_corner_down+unit_steps)/6
    return distances_score
...
def evaluation(obs, player_x, player_y, teammate_x, teammate_y, teammate_speed_x=0, teammate_speed_y=0, passing_steps=SHORT_STEPS):
    if is_blocked(obs, player_x, player_y, teammate_x, teammate_y, teammate_speed_x=teammate_speed_x, teammate_speed_y=teammate_speed_y,  speed=passing_steps): return False
...
def bad_angle_short_pass(obs, player_x, player_y):
    ...
    def environment_fits(obs, player_x, player_y):
        ...
        if player_x > (1-0.1125*1.5) and abs(player_y) > (0.42-0.1125*1.5): return True
        else: return False
...
def close_to_goalkeeper_shot(obs, player_x, player_y):
    ...
    def environment_fits(obs, player_x, player_y):
        ...
        if get_distance(player_x, player_y, goalkeeper_x, goalkeeper_y) < 0.3:
            return True
        if get_distance(player_x, player_y, 1, 0) < 0.3:
            return True
        return False
...
def close_to_opponent_pass(obs, player_x, player_y):
    ...
    def environment_fits(obs, player_x, player_y):
        """ environment fits constraints """
        short_pass = [-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999]
        long_pass = [-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999]
        high_pass = [-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999]
        ...
        check5 = [False, False, False, False, False, False, False, False, False, False, False, False, ]
        for j in range(0, len(obs["left_team"])):
            ...
            if min_distance_to_opponent > RUN_SPEED*2 and not check1 and not check2 and not check3 and not check4: 
                check5[j] = True
            if check1 or check2 or check3 or check4 or check5[j]:
                distance_to_teammate = get_distance(player_x, player_y, obs["left_team"][j][0], obs["left_team"][j][1])
                if distance_to_teammate < 1/2 and min_distance_to_opponent > RUN_SPEED:
                    evaluation_score = evaluation(obs, player_x, player_y, obs["left_team"][j][0], obs["left_team"][j][1], teammate_speed_x=obs["left_team_direction"][j][0], teammate_speed_y=obs["left_team_direction"][j][1], passing_steps=SHORT_STEPS)
                    if evaluation_score: short_pass[j] = evaluation_score

                if distance_to_teammate < 1 and distance_to_teammate > 1/2 and min_distance_to_opponent > RUN_SPEED and j != 0:
                    evaluation_score = evaluation(obs, player_x, player_y, obs["left_team"][j][0], obs["left_team"][j][1], teammate_speed_x=obs["left_team_direction"][j][0], teammate_speed_y=obs["left_team_direction"][j][1], passing_steps=LONG_STEPS)
                    if evaluation_score: 
                        if j != 0: long_pass[j] = evaluation_score
                    
                if distance_to_teammate < 1.5 and distance_to_teammate > 1/2 and min_distance_to_opponent > RUN_SPEED and j != 0:
                    evaluation_score =  evaluation(obs, player_x, player_y, obs["left_team"][j][0], obs["left_team"][j][1], teammate_speed_x=obs["left_team_direction"][j][0], teammate_speed_y=obs["left_team_direction"][j][1], passing_steps=HIGH_STEPS)
                    if evaluation_score: 
        ...
        if best_action == 0: teammate_idx = np.argmax(short_pass)
        if best_action == 1: teammate_idx = np.argmax(long_pass)
        if best_action == 2: teammate_idx = np.argmax(high_pass)
        if check5[teammate_idx]: 
...
def go_through_opponents(obs, player_x, player_y):
    ...
    def environment_fits(obs, player_x, player_y):
        ...
        for step in range(10):
            ...
            right_score += 1/10 * get_player_score(obs, future_x + horizontal_acceleration, future_y, steps=-2/3, routing=True)
            top_right_score += 1/10 * get_player_score(obs, future_x + diagonal_acceleration, future_y - diagonal_acceleration, steps=-1/3, routing=True)
            bottom_right_score += 1/10 * get_player_score(obs, future_x + diagonal_acceleration, future_y + diagonal_acceleration, steps=-1/3, routing=True)
            ...
            top_left_score += 1/10 * get_player_score(obs, future_x - diagonal_acceleration, future_y - diagonal_acceleration, steps=1/3, routing=True)
            bottom_left_score += 1/10 * get_player_score(obs, future_x - diagonal_acceleration, future_y + diagonal_acceleration, steps=1/3, routing=True)
            left_score += 1/10 * get_player_score(obs, future_x - horizontal_acceleration, future_y, steps=2/3, routing=True)
        ...
        # if not is_within_border(obs, player_x, 0, xborder=-1, yborder=None) or not is_within_border(obs, obs["ball"][0], 0, xborder=1, yborder=None): obs["memory_patterns"]["go_around_opponent"] = Action.HighPass
        # if not is_within_border(obs, player_x, 0, xborder=1, yborder=None) or not is_within_border(obs, obs["ball"][0], 0, xborder=1, yborder=None): obs["memory_patterns"]["go_around_opponent"] = Action.HighPass
        # if not is_within_border(obs, 0, player_y, xborder=None, yborder=0.42) or not is_within_border(obs, 0, obs["ball"][1], xborder=None, yborder=0.42): obs["memory_patterns"]["go_around_opponent"] = Action.HighPass
        # if not is_within_border(obs, 0, player_y, xborder=None, yborder=-0.42) or not is_within_border(obs, 0, obs["ball"][1], xborder=None, yborder=-0.42): obs["memory_patterns"]["go_around_opponent"] = Action.HighPass
