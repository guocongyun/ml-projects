...
def check_block(obs, player_x, player_y, teammate_x, teammate_y):
    ball_pos = [player_x, player_y]
    distance = normal_get_distance(player_x, player_y, teammate_x, teammate_y)
    steps = int(round(distance / PASS_SPEED))
    ...
    for step in range(steps):
        ...
        for opponent in range(1, len(obs["right_team"])):
        ...
        for i in range(1, len(obs["right_team"])):
            ...
            if distance_to_opponent < 0.03:
                for j in range(1, len(obs["left_team"])):
                    ...
                    if distance_to_teammate < 0.2 and player_to_opponents[0] < teammate_to_opponents[0] and check_legal_move(obs, player_x, player_y) and check_block(obs, player_x, player_y, obs["left_team"][j][0], obs["left_team"][j][1]):
        ...
        for i in range(1, len(obs["right_team"])):
            ...
            if distance_to_opponent < 0.06 and distance_to_opponent > 0.03:
                for j in range(1, len(obs["left_team"])):
                    ...
                    teammate_to_opponents = get_average_distance_to_opponents(obs, obs["left_team"][j][0], obs["left_team"][j][1])
                    distance_to_teammate = get_distance(player_x, player_y, obs["left_team"][j][0], obs["left_team"][j][1])
                    if distance_to_teammate < 0.4 and distance_to_teammate > 0.2 and player_to_opponents[0] < teammate_to_opponents[0] and check_legal_move(obs, player_x, player_y) and check_block(obs, player_x, player_y, obs["left_team"][j][0], obs["left_team"][j][1]):
                        teammate_distance_to_goal = get_distance(obs["left_team"][j][0], obs["left_team"][j][1], 1, 0)
                        player_distance_to_goal = get_distance(player_x, player_y, 1, 0)
        ...
        for i in range(1, len(obs["right_team"])):
            ...
            if distance_to_opponent > 0.06 and distance_to_opponent < 0.09:
                for j in range(1, len(obs["left_team"])):
                    ...
                    teammate_to_opponents = get_average_distance_to_opponents(obs, obs["left_team"][j][0], obs["left_team"][j][1])
                    distance_to_teammate = get_distance(player_x, player_y, obs["left_team"][j][0], obs["left_team"][j][1])
                    if distance_to_teammate < 0.6 and distance_to_teammate > 0.4 and player_to_opponents[0] < teammate_to_opponents[0] and check_legal_move(obs, player_x, player_y) and check_block(obs, player_x, player_y, obs["left_team"][j][0], obs["left_team"][j][1]):
                        teammate_distance_to_goal = get_distance(obs["left_team"][j][0], obs["left_team"][j][1], 1, 0)
                        player_distance_to_goal = get_distance(player_x, player_y, 1, 0)
