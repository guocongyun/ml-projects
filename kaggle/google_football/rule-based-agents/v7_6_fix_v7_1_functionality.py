...
def check_block(obs, player_x, player_y, teammate_x, teammate_y, speed=PASS_SPEED):
    ...
    for step in range(steps):
        ...
        for opponent in range(0, len(obs["right_team"])):
            distance_to_opponent = normal_get_distance(ball_pos[0], ball_pos[1], obs["right_team"][opponent][0], obs["right_team"][opponent][1]) + RUN_SPEED*step/2
        ...
        for j in range(0, len(obs["left_team"])):
            ...
            if distance_to_teammate < 1/2:
                ...
                if check1 or check2 or check3 or check4:
                    if evaluation(obs, player_x, player_y, obs["left_team"][j][0], obs["left_team"][j][1]):
                        return True
        return False
