...
def normal_get_distance_to_opponent(obs, player_x, player_y):
    """ get average distance to closest opponents """
    distances_score = 0
    distances_amount = 0
    for i in range(1, len(obs["right_team"])):
        ...
        if distance_to_opponent < RUN_SPEED*1: distances_score -= 9**20*damping_factor/20; distances_amount += 1
        elif distance_to_opponent < RUN_SPEED*2: distances_score -=9**19*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*3: distances_score -=9**18*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*4: distances_score -=9**17*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*5: distances_score -=9**16*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*6: distances_score -=9**15*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*7: distances_score -=9**14*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*8: distances_score -=9**13*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*9: distances_score -=9**12*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*10: distances_score -=9**11*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*11: distances_score -=9**10*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*12: distances_score -=9**9*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*13: distances_score -=9**8*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*14: distances_score -=9**7*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*15: distances_score -=9**6*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*16: distances_score -=9**5*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*17: distances_score -=9**4*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*18: distances_score -=9**3*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*19: distances_score -=9**2*damping_factor/20
        elif distance_to_opponent < RUN_SPEED*20: distances_score -=9**1*damping_factor/20
    if distances_amount == 0:
...
def go_through_opponents(obs, player_x, player_y):
    ...
    def environment_fits(obs, player_x, player_y):
        ...
        right, opponents_amount = normal_get_distance_to_opponent(obs, player_x + RUN_SPEED, player_y)
        obs["memory_patterns"]["go_around_opponent"] = Action.Right
        # if top right direction is safest
        top_right, opponents_amount = normal_get_distance_to_opponent(obs, player_x + math.sqrt(RUN_SPEED), player_y - math.sqrt(RUN_SPEED))
        bottom_right, opponents_amount = normal_get_distance_to_opponent(obs, player_x + math.sqrt(RUN_SPEED), player_y + math.sqrt(RUN_SPEED))
        top_left, opponents_amount = normal_get_distance_to_opponent(obs, player_x - math.sqrt(RUN_SPEED), player_y - math.sqrt(RUN_SPEED))
        bottom_left, opponents_amount = normal_get_distance_to_opponent(obs, player_x - math.sqrt(RUN_SPEED), player_y + math.sqrt(RUN_SPEED))
        top, opponents_amount = normal_get_distance_to_opponent(obs, player_x, player_y + RUN_SPEED)
        bottom, opponents_amount = normal_get_distance_to_opponent(obs, player_x, player_y + RUN_SPEED)

        possible_movement = [right/3]

        if player_y > -0.3: possible_movement.append(top_right/3)
        if player_y < 0.3: possible_movement.append(bottom_right/3)
        if player_y > -0.238: possible_movement.append(top*2/3)
        if player_y < 0.238: possible_movement.append(bottom*2/3)
        if player_y > 0.3: possible_movement.append(top_left)
        if player_y < 0.3: possible_movement.append(bottom_left)
        ...
        if (top == 999 and player_y > -0.375): obs["memory_patterns"]["go_around_opponent"] = Action.TopLeft
        if (bottom == 999 and player_y < 0.375): obs["memory_patterns"]["go_around_opponent"] = Action.BottomLeft        
        if (top == 999 and player_y > -0.345): obs["memory_patterns"]["go_around_opponent"] = Action.Top
        if (bottom == 999 and player_y < 0.345): obs["memory_patterns"]["go_around_opponent"] = Action.Bottom
        if (top_right == 999 and player_y > -0.375): obs["memory_patterns"]["go_around_opponent"] = Action.TopRight
        if (bottom_right == 999 and player_y < 0.375): obs["memory_patterns"]["go_around_opponent"] = Action.BottomRight
        if top_right == 999: obs["memory_patterns"]["go_around_opponent"] = Action.Right
