...
def distance_to_opponents(obs, player_x, player_y):
    PLAYER_ACCELERATION = 0.0035
    PLAYER_SPEED = 0.0035
    steps = quadratic_equation_solver(0.00, obs["ball_direction"][2], (obs["ball"][2] - 1))
    return steps
...
#%%
            ...
            for step in range(5):
                distances_to_opponent.append(get_distance(player_x + obs["right_team"][i][0], player_y, obs["right_team"][i][0], obs["right_team"][i][1]))
            distance_to_opponent = min(distances_to_opponent)
#%%
            if distance_to_opponent < 0.03:
