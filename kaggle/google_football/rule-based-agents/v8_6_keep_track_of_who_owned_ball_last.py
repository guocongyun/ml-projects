...
def offence_memory_patterns(obs, player_x, player_y):
    ...
    def environment_fits(obs, player_x, player_y):
        ...
        for i in range(0, len(obs["right_team"])):
            distance_to_opponent = normal_get_distance(obs["ball"][0], obs["ball"][1], obs["right_team"][i][0], obs["right_team"][i][1])
            distance_to_player = normal_get_distance(obs["ball"][0], obs["ball"][1], player_x, player_y)
            if last_ball_owner == 0 and distance_to_opponent < distance_to_player + RUN_SPEED * 5:
                opponent_close = True
            if last_ball_owner == 1 and distance_to_opponent < distance_to_player + RUN_SPEED * 10:
                opponent_close = True
