...
def distance_to_corner(obs, player_x, player_y, steps=0):
    ...
    if future_y < 0:
        ...
        if future_x < -0.4:
            distances_score = 10**(1-distance_to_corner_up+unit_steps)*5/6 + 10**(1-distance_to_self_corner_up+unit_steps)/6
    if future_y >= 0: 
        if future_x > -0.4:
...
def evaluation(obs, player_x, player_y, teammate_x, teammate_y, teammate_speed_x=0, teammate_speed_y=0, passing_steps=SHORT_STEPS):
    ...
    if not check_legal_move(obs, teammate_x, teammate_y): return False
