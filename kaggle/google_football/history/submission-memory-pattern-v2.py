...
# PLAYER_GOAL_RANGE = -0.3
PLAYER_GOAL_RANGE = 0
PASS_DISTANCE_OPPONENT = 0
PASS_DISTANCE_TEAMMATE = 0
# ROUNTING_DISTANCE = -0.02
ROUNTING_DISTANCE = 0.1
# ROUNTING_DISTANCE = 0
...
def close_to_goalkeeper_shot(obs, player_x, player_y):
    # TODO implement short pass for goaly
...
def close_to_opponent_short_pass(obs, player_x, player_y):
    ...
    def environment_fits(obs, player_x, player_y):
        ...
        for i in range(1, len(obs["right_team"])):
            distance_to_opponent = get_distance(player_x, player_y, obs["right_team"][i][0], obs["right_team"][i][1])
            if distance_to_opponent < 0.03 + PASS_DISTANCE_OPPONENT:
                for j in range(1, len(obs["left_team"])):
                    ...
                    if distance_to_teammate < 0.2 + PASS_DISTANCE_TEAMMATE:
...
def far_from_goal_shot(obs, player_x, player_y):
    ...
    def environment_fits(obs, player_x, player_y):
        ...
        if player_x < -0.6 + PLAYER_GOAL_RANGE or obs["ball_owned_player"] == 0:
            return True
        return False
...
def far_from_goal_high_pass(obs, player_x, player_y):
    ...
    def environment_fits(obs, player_x, player_y):
        """ environment fits constraints """
        # player is far from opponent's goal or it's the goalkeeper
        if player_x < -0.3 + PLAYER_GOAL_RANGE/2 or obs["ball_owned_player"] == 0:
            return True
        return False
...
def go_through_opponents(obs, player_x, player_y):
    ...
    def environment_fits(obs, player_x, player_y):
        ...
        if (top_right > biggest_distance and player_y > -0.15 - ROUNTING_DISTANCE) or (top_right == 2 and player_y > 0.07):
        ...
        if (bottom_right > biggest_distance and player_y < 0.15 + ROUNTING_DISTANCE) or (bottom_right == 2 and player_y < -0.07):
