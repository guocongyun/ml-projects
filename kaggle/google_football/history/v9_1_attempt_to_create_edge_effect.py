...
def distance_to_opponent(obs, player_x, player_y, steps=0, passing_from=False):
    """ get average distance to closest opponents """
    distances_score = 0
    for i in range(1, len(obs["right_team"])):
        damping_factor = 1
        ...
        if not passing_from: distances_score += -10**((10-step_to_opponent+steps/2)*damping_factor/10)
        if passing_from: distances_score += -10**((10-step_to_opponent)*damping_factor/10)

    return distances_score

def distance_to_goal(obs, player_x, player_y, steps=0):
    unit_distance = 1/10
    distance_to_goal_up = get_distance(player_x, player_y, 1, -0.044)
    distance_to_goal_mid = get_distance(player_x, player_y, 1, 0)
    distance_to_goal_down = get_distance(player_x, player_y, 1, 0.044)

    distance_to_horizontal_edge = abs(0.9 - abs(player_x))
    distance_to_vertical_edge = abs(0.32 - abs(player_y))

    distance_to_goal = min([distance_to_goal_up, distance_to_goal_mid, distance_to_goal_down])
    distance_to_goal = distance_to_goal * 2/3 + distance_to_horizontal_edge/6 + distance_to_vertical_edge/6
    step_to_goal = distance_to_goal
    unit_steps = (steps*RUN_SPEED)
    distances_score = 2*(11**((1-step_to_goal+unit_steps)))
    return distances_score

def get_player_score(obs, player_x, player_y, steps=0, passing_from=False):
    player_score = 0
    player_score += distance_to_opponent(obs, player_x, player_y, steps, passing_from=passing_from)
    player_score += distance_to_goal(obs, player_x, player_y, steps)
    return player_score
...
def evaluation(obs, player_x, player_y, teammate_x, teammate_y, passing_steps=SHORT_STEPS):
    ...
    player_score = get_player_score(obs, player_x, player_y, steps=player_steps_to_teammate, passing_from=True)
    teammate_score = get_player_score(obs, teammate_x, teammate_y, steps=player_steps_to_teammate, passing_from=False)
...
def close_to_opponent_short_pass(obs, player_x, player_y):
    ...
    def environment_fits(obs, player_x, player_y):
        ...
        for j in range(0, len(obs["left_team"])):
            ...
            if 1/4 < distance_to_teammate < 1/2:
