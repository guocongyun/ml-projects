...
def build_mlp(
  ...
  with tf.variable_scope(scope):
    ...
    for layer in range(n_layers):
      x = tf.keras.layers.Dense(size, activation='relu').apply(x)
    out = tf.keras.layers.Dense(output_size, activation=output_activation).apply(x)
...
class PG(object):
  ...
  def add_placeholders_op(self):
    ...
    if self.discrete:
      ...
      # self.action_placeholder = tf.placeholder(tf.float32, (None,))
  ...
  def build_policy_network_op(self, scope = "policy_network"):
    ...
    if self.discrete:
      # IMPORTANT the input is the self.observation 
      action_logits = build_mlp(self.observation_placeholder, self.action_dim, scope, self.config.n_layers, self.config.layer_size)
      ...
      #   trainable = True
      # )
      # # IMPORTANT # Reparameterize / soft resampling: miu + sigma * N(0,1) rather than N(miu,sigma) to expose miu and sigma to backpropagation
      # self.sampled_action = tf.exp(log_std) * tf.random_normal((self.config.batch_size, self.action_dim)) + action_means
      # distribution = tfp.distributions.MultivariateNormalDiag(action_means,  tf.exp(log_std)) 
      ...
      self.logprob = tfp.distributions.MultivariateNormalDiag(action_means, log_std).log_prob(self.action_placeholder)  # TODO
    #######################################################
    #########          END YOUR CODE.          ############
  ...
  def add_loss_op(self):
    ...
    # Reduce_mean is tf's version of mean 
    # IMPORTANT - in the bracket because gradient descent is ceta = ceta - α ∇ loss
  ...
  def get_returns(self, paths):
      ...
      for path in paths:
          ...
          returns[-1] = rewards[-1]
          # IMPORTANT calculate returns in reverse order!!
  ...
  def calculate_advantage(self, returns, observations):
      ...
      #######################################################
      #########   YOUR CODE HERE - 5-10 lines.   ############
      if config.use_baseline: adv -= self.sess.run(self.baseline, feed_dict={self.observation_placeholder:observations})
      if config.normalize_advantage: adv = (adv - np.mean(adv)) / (np.std(adv) + 1e-12)
...
if __name__ == '__main__':
  ...
  config = get_config(args.env_name, True)
